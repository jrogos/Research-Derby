...
PPdepth_func <- function(PPmax, Izt, Ik = 217.5, Vzdel, daylength) {
  ...
  # PPmax <- WQ_PAR_cone$PP_max; daylength <- WQ_PAR_cone$daylight; Izt <- WQ_PAR_cone$Izt; Vzdel <- WQ_PAR_cone$VOL_M3_CROSS_SECTION
  ...
  # (daylength)*sum(PPmax*tanh(Izt/217.5)*(Vzdel), na.rm = TRUE)
